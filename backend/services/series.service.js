...
exports.deleteById = async (req) => {
 try {
  ...
  if (!isPosterDeleted || !isDeletedBannerPoster) {
   ...
   await Promise.all([
    helpers.deleteDatafilter(findedDirectors, directorDal,findedSeries._id),
    helpers.deleteDatafilter(findedGenres, genreDal,findedSeries._id),
    helpers.deleteDatafilter(findedStars, starDal,findedSeries._id),
    helpers.deleteDatafilter(findedScriptwriters, scriptwriterDal,findedSeries._id)
   ]);
